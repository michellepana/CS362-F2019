...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case baron:
        ...
        baronCard(state, choice1, currentPlayer, estate);
        /*
    ...
    */
    ...
    case minion:
        //+1 action
        minionCard(state, handPos, currentPlayer, choice1, choice2);
        /*
        state->numActions++;
 ...
 */
    case ambassador:
        ambassadorCard(choice1, choice2, currentPlayer, handPos, state);
        /*
...
int baronCard(struct gameState *state, int choice1, int currentPlayer, int estate){
    printf("---->Starting baron function!\n");
    //state->numBuys++;//Increase buys by 1!
    if (choice1 > 0) { //Boolean true or going to discard an estate
        int p = 0;//Iterator for hand!
        int card_not_discarded = 1;//Flag for discard set!
...
int minionCard(struct gameState *state, int handPos, int currentPlayer, int choice1, int choice2){
    printf("---->Starting minionCard function!\n");
    int i;
    int j;
    state->numActions++;
    ...
    else if (choice2)		//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
    {
        ...
        for (i = 0; i < state->numPlayers; i++)
        {
            if (i != currentPlayer)
            {
                //if ( state->handCount[i] > 4 )//introducing a bug that doesn't check if other player's hand has more than 4 cards
                //{
                    //discard hand
                    while( state->handCount[i] > 0 )
                    {
...
int tributeCard(struct gameState *state, int nextPlayer, int currentPlayer, int *tributeRevealedCards){
    printf("---->Starting tributeCard function!\n");
    int i;
...
int ambassadorCard(int choice1, int choice2, int currentPlayer, int handPos, struct gameState *state){
    ...
    printf("---->Starting ambassadorCard function!\n");
    int i;
    int j = 0;		//used to check if player has enough cards to discard
    //Introduing bug here, does not bound the player's choice to an upper limit of 2 and lower limit of 0
    /*
    if (choice2 > 2 || choice2 < 0)
    {
        return -1;
    ...
    */
    ...
    for (i = 0; i < state->numPlayers; i++)
    {
        //Introducing a bug here that the current player will also gain a copy of the revealed card
        //if (i != currentPlayer)
        //{
            gainCard(state->hand[currentPlayer][choice1], state, 0, i);
        //}
