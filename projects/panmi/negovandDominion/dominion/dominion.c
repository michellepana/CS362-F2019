...
//baron effect for cardEffect switch statement
...
//minions effect for cardEffect switch statement
int minionEffect(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer) {
    ...
    else if (choice2)		//discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
    {
        ...
        {
            discardCard(currentPlayer, handPos, state, 0); // BUG: switched handPos and currentPlayer spots in call
        }
        ...
        for (i = 0; i <= state->numPlayers; i++) // BUG: changed i = to i <=
        {
            if (i != currentPlayer)
            {
...
int ambassadorEffect(int choice1, int choice2, struct gameState *state, int handPos) {
    ...
    if (choice2 > 2 && choice2 < 0) //BUG:  changed || to AND
    {
        return -1;
    }
    ...
    for (i = 0; i < state->numPlayers; i++)
    {
        if (i != currentPlayer)
        {
            gainCard(state->hand[currentPlayer][choice2], state, 0, i); //BUG: changed choice1 to choice2
        }
    }
...
int tributeEffect(struct gameState *state) {
    ...
    if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 1) {
        ...
        for (i = 0; i <= 2; i ++) {
            ...
            //BUG changed from else if to if
    ...
    int j = state->hand[currentPlayer][choice1];  //store card we will trash
        ...
        for (i = 0; i < state->handCount[currentPlayer]; i++)
        {
            if (state->hand[currentPlayer][i] == j)
            {
                ...
                //break;  //BUG:: removed break statement
            }
        }
