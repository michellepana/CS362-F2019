...
gameTest.hand[0][1] = mine;            //card to reveal in pos 1
gameTest.hand[0][2] = ambassador;      //ambassador in position 2

gameTest.handCount[1] = 2;
gameTest.hand[1][0] = copper;
gameTest.hand[1][1] = copper;                 

gameTest.handCount[2] = 4;
gameTest.hand[2][0] = copper;
gameTest.hand[2][1] = copper;           
gameTest.hand[2][2] = copper;    
gameTest.hand[2][3] = copper; 

int preSupply = gameTest.supplyCount[mine];
ambassadorEffect(1, 0, &gameTest, 2);  //call function

printf("player should be down to 2 cards\n");
asserttrue(gameTest.handCount[0],2);
printf("other players should have 1 more card(mine) than before");
asserttrue(gameTest.handCount[1],3);
asserttrue(gameTest.handCount[2], 5);
asserttrue(gameTest.hand[1][2], mine);
asserttrue(gameTest.hand[2][4], mine);



//ambassadorCard(choice1, choice2, thisPlayer, handpos, &gameTest);
ambassadorEffect(choice1, choice2, &gameTest, handpos);
int returnedCard = 2;
...
asserttrue(ambassadorEffect(choice1, choice2, &gameTest, handpos), result);
